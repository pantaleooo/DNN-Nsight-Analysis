...
本项目是一个为 PyTorch 设计的、用于深度神经网络 (DNN) 模型在视频流上进行推理或训练的性能分析框架。

它深度集成了 NVIDIA Nsight Systems (nsys) 和 NVTX 标记，允许对复杂的视频处理流水线 (从数据加载、预处理到模型执行) 进行逐帧的瓶颈分析。

✨ 核心功能

📈 深度性能剖析: 在 worker.py, models.py, data_pipeline.py 中深度集成了 torch.cuda.nvtx 标记，可与 nsys 结合使用，精确定位 GPU/CPU 瓶颈。

⛓️ 混合流水线: 支持灵活地将预处理 (--prep-on-gpu) 和模型执行 (--infer-on-gpu) 分配到 CPU 或 GPU 运行，以测试不同硬件配置下的性能。
...
🎯 CPU 亲和性: 支持将 Worker 进程绑定到特定的 CPU 核心 (--core-bind)，以减少多进程间的资源抢占，确保分析结果的稳定性 (见 utils.py)。
...
📊 指标导出: 自动将每个批次的详细延迟（读取、预处理、执行）和损失（训练模式下）记录到唯一的 CSV 文件中，便于后续分析 (见 utils.MetricsCollector)。

🔬 实验性功能: 包含一个 SplitModelWrapper (见 models.py)，用于实验性地将模型（如 ResNet）在 CPU 和 GPU 之间进行跨层分割。

🛠️ 辅助工具: 额外提供了 frame_extract/ 目录，用于从视频中提取帧 (frame_extractor.py) 并使用高精度模型进行目标检测标注 (image_annotator.py)。
...
(可选) 确保已安装 NVIDIA Nsight Systems (nsys)，以便进行深度性能剖析。
...
preprocess_batch: 关键函数。它根据 prep_on_gpu 标志决定是在 CPU 上执行 cpu_pipeline，还是将原始张量传输到 GPU (prep_device) 并执行 gpu_prep_ops。
...
Nsight 报告 (.qdrep) (如果使用 nsys 运行):

用于在 Nsight Systems GUI 中进行最深入的瓶颈分析，查看 CUDA kernel、NVTX 范围和 CPU/GPU 交互。
